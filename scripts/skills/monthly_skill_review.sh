...
python3 - "$REGISTRY" "$OUT_FILE" "$MONTH" <<'PY'
import sys
...
registry, out_file, month = sys.argv[1:]
...
lines = [
    ...
    "## 全量清单",
    "",
    "| skill_id | lifecycle | default_enabled | blast_radius | 建议 |",
    "|---|---|---|---|---|",
]
...
for s in skills:
    ...
    if blast == "external_side_effect" and lifecycle != "verified":
        suggestion = "优先隔离/降级"
    lines.append(f"| {sid} | {lifecycle} | {default_enabled} | {blast} | {suggestion} |")
